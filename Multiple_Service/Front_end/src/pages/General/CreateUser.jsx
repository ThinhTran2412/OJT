    ...
    const fetchRoles = async () => {
      try {
        ...
        const response = await api.get('/Roles', {
          params: {
            Page: 1,
            PageSize: 100, // Lấy tối đa 100 roles
            SortBy: 'Name',
            SortDesc: false
          }
        });
        // API trả về dạng { items: [...], page: 1, pageSize: 100, total: 7 }
        ...
        if (status === 401 || status === 403) {
          // Không có quyền truy cập - chỉ Admin mới được tạo user
          setErrorMessage('Access denied. Only administrators can create users.');
          setErrorType('unauthorized');
          // Có thể redirect về dashboard hoặc ẩn form
          setTimeout(() => {
            navigate('/dashboard');
          }, 3000);
        } else {
          // Lỗi khác - hiển thị chi tiết lỗi để debug
