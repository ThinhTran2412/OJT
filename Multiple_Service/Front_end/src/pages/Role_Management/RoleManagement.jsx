...
export default function RoleManagement() {
  const navigate = useNavigate(); // ✅ để điều hướng sang trang Update Role
  ...
  const fetchRoles = useCallback(
    async (filterParams = filters) => {
      ...
      try {
        ...
        else setError(error.message || 'Không thể tải danh sách vai trò');
      } finally {
        setLoading(false);
      }
    },
  ...
  // ✅ Delete role handler (hiển thị thông báo thay vì chuyển trang)
  const handleDeleteRole = useCallback(
    async (roleId) => {
      try {
        ...
        // Luôn refresh danh sách hiện tại
        fetchRoles(filters);
      }
    },
    [fetchRoles, filters]
    ...
    <div className="flex items-center justify-between">
      ...
      <p className="text-yellow-700 text-sm">Error loading privileges.</p>
        ...
        <RoleTable
          ...
          onEditRole={handleEditRole} // ✅ truyền callback Edit
          onDeleteRole={handleDeleteRole} // ✅ truyền callback Delete (tùy chọn)
        />
      </div>
    </DashboardLayout>
  );
