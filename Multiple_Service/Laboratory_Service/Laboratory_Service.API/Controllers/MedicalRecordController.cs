...
namespace Laboratory_Service.API.Controllers
{
    ...
    public class MedicalRecordController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateMedicalRecord(int id, [FromBody] UpdateMedicalRecordCommand request)
        {
            ...
            var allowedRoles = new[] { "1", "2", "3" };
            if (userRoles == null || !userRoles.Any(r => allowedRoles.Contains(r)))
                return Forbid("You are not authorized to update medical records.");
            var userName = User?.FindFirst(ClaimTypes.Name)?.Value;
            var userId = User?.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            var identifyNumberClaim = User?.FindFirst("identifyNumber")?.Value;
            request.UpdatedBy = !string.IsNullOrEmpty(userName) ? userName :
                                !string.IsNullOrEmpty(userId) ? userId :
                                !string.IsNullOrEmpty(identifyNumberClaim) ? identifyNumberClaim : request.UpdatedBy;
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new { message = "An error occurred while updating the medical record." });
            }
        }
