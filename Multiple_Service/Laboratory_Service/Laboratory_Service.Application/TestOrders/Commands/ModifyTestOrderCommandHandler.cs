...
{
    /// <summary>
    /// Handle Modify Test Order Command
    /// </summary>
    ...
    public class ModifyTestOrderCommandHandler : IRequestHandler<ModifyTestOrderCommand, bool>
    {
        ...
        public async Task<bool> Handle(ModifyTestOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            if (order.PatientId != syncedPatient.PatientId)
            {
                order.PatientId = syncedPatient.PatientId;
                _logger.LogInformation("Re-linked TestOrder to PatientId {PatientId}", syncedPatient.PatientId);
            }

            var existingRecords = await _recordRepo.GetByPatientIdAsync(syncedPatient.PatientId);
            ...
            order.MedicalRecordId = medicalRecord.MedicalRecordId;
            order.PatientName = request.PatientName;
            order.DateOfBirth = request.DateOfBirth;
            ...
            await _eventLogRepo.CreateAsync(new EventLog
            {
                ...
                EventLogMessage = $"Test Order {order.OrderCode} modified by {request.UpdatedByName}",
                OperatorId = request.UpdatedBy,
                OperatorName = request.UpdatedByName,
