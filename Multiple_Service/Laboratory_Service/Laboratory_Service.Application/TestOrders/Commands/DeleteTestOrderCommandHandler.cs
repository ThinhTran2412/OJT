...
namespace Laboratory_Service.Application.TestOrders.Commands
{
    ...
    public class DeleteTestOrderCommandHandler : IRequestHandler<DeleteTestOrderCommand, bool>
    {
        ...
        {
            var testOrder = await _repository.GetByIdAsync(request.TestOrderId);
            if (testOrder == null)
            ...
            var operatorId = request.DeletedBy;
            var operatorName = request.DeletedByName;

            await _eventLogService.CreateAsync(new EventLog
            {
                EventId = "E_00003",
                Action = "Delete Test Order",
                EventLogMessage = $"Test Order {testOrder.OrderCode} deleted by {operatorName}",
