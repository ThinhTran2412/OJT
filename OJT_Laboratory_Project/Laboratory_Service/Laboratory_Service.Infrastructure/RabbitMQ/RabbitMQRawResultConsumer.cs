...
namespace Laboratory_Service.Infrastructure.RabbitMQ
{
    ...
    public class RabbitMQRawResultConsumer : BackgroundService
    {
        ...
        public RabbitMQRawResultConsumer(IServiceProvider serviceProvider, ILogger<RabbitMQRawResultConsumer> logger)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("RabbitMQ Consumer connected and queue '{QueueName}' declared. Host: {Host}, Username: {Username}", 
                    RabbitMQSettings.Queue_Raw_From_Monitoring, RabbitMQSettings.Host, RabbitMQSettings.Username);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "FATAL ERROR: Could not connect to RabbitMQ Host: {HostName}. Consumer will not start processing messages.", RabbitMQSettings.Host);
            }
        }
