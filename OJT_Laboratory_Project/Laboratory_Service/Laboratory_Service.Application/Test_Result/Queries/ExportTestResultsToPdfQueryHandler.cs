...
namespace Laboratory_Service.Application.Test_Result.Queries
{
    ...
    public class ExportTestResultsToPdfQueryHandler : IRequestHandler<ExportTestResultsToPdfQuery, byte[]>
    {
        ...
        public ExportTestResultsToPdfQueryHandler(ITestOrderRepository testOrderRepository)
        {
                        ...
                        column.Item()
                            ...
                            .Table(table =>
                            {
                                ...
                                AddInfoRow("Created By:", testOrder.CreatedBy ?? "N/A",
                                    "Run By:", testOrder.RunBy?.ToString() ?? "N/A");

                                AddInfoRow("Created At:", testOrder.CreatedAt.ToString("dd/MM/yyyy HH:mm"),
                                    "Run Date:", testOrder.RunDate != default
                ...
                container
                    .Column(column =>
                    {
                        ...
                        if (resultsWithComments.Any())
                        {
                            column.Item()
                                .PaddingTop(15)
                                .Element(CommentsSection(resultsWithComments));
                        }
                    });
            };
        }

        private static Action<IContainer> CommentsSection(List<TestResult> resultsWithComments)
        {
            return container =>
            {
                container
