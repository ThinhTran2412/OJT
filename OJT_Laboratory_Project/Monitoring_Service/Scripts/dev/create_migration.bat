...
REM Change to Monitoring_Service directory (go up two levels from dev folder)
cd /d "%~dp0\..\.."
...
if not exist "Monitoring_Service.API\Monitoring_Service.API.csproj" (
    ...
    powershell -Command "Write-Host 'Please run this script from Monitoring_Service\Scripts\dev folder.' -ForegroundColor Yellow"
    exit /b 1
)

powershell -Command "Write-Host ''"
powershell -Command "Write-Host '==============================================================' -ForegroundColor Cyan"
powershell -Command "Write-Host '  Creating Monitoring_Service Migration (DEVELOPMENT)' -ForegroundColor Cyan"
